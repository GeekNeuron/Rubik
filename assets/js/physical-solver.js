// Bridges the app to its own from-scratch solver (own-solver.js - see that
// file for how it works and why it's original code, not a third-party
// library), which runs inside a Web Worker so solving never blocks the UI
// thread. Also holds the facelet-string construction logic used by the
// "solve my physical cube" feature.
 ...
 * row-major) using this app's own from-scratch solver (own-solver.js).
 * Resolves with an array of moves in STANDARD notation (e.g. "R2","U'","F"),
 * not yet translated to this app's internal engine format. Solutions run
 * ~70-130 moves (this trades optimality for the solver being fully
 * original code) and can occasionally take up to ~30s on harder scrambles,
 * hence the generous default timeout.
 */
export function solveFacelets(faceletString, timeoutMs = 45000) {
 ...
 * facelet string this app's solver expects, after remapping each physical
 * color to the canonical U/R/F/D/L/B letter of whichever face currently
 * has that color at its center.
 */
