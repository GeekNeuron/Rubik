...
function getRoundedCubieGeometry() {
    if (!sharedRoundedGeometry) {
        sharedRoundedGeometry = createRoundedBoxGeometry(CUBIE_SIZE, CORNER_RADIUS, GEOMETRY_SEGMENTS);
    }
    return sharedRoundedGeometry.clone();
}

/**
 * Builds an actual rounded-corner box geometry (not a flat box with a
 * "rounded-looking" texture): starts from a subdivided BoxGeometry, then
 * inflates each vertex outward - vertices deep inside a face stay put,
 * while vertices near an edge/corner get pulled toward a sphere of the
 * given radius, using the standard "rounded box = inner box (+) sphere"
 * (Minkowski sum) construction. Segments must be high enough for the
 * rounded edges to look smooth rather than faceted.
 */
...
function createVisualCubie(x, y, z) {
    const geometry = getRoundedCubieGeometry();
...
function getStickerTexture(cssColor) {
    ...
    // Base sticker fill, inset slightly from the tile edge so a thin sliver
    // of the underlying (dark) plastic material shows as a seam - matching
    // how real cube stickers don't quite reach the piece's edges.
...
export function updateCubeColors() {
    ...
    cachedCubeGroup.children.forEach(cubie => {
        ...
        cubie.material.forEach((mat, i) => {
            ...
            mat.map = materials[i].map || null; // re-point at the (cached) texture for the new color
