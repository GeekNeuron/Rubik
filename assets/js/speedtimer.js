...
export function createTimerController({ onStateChange, onTick, onInspectionTick, onRecorded }) {
    ...
    // --- Hardware (StackMat-over-microphone) input path -----------------
    // Mirrors the manual hold/release flow above but is driven by real
    // packets from the physical timer instead of pointer/keyboard events:
    // no artificial HOLD_MS delay is needed since placing both hands on
    // real hardware *is* the "hold". Shares the same session/stats/
    // inspection state so times recorded via microphone and via manual
    // tap-timing appear in one unified history.
    function hardwareReady() {
        if (state === 'running') return; // stray/duplicate packet - ignore
        if (state === 'inspecting') {
            const elapsed = performance.now() - inspectionStart;
            pendingPenalty = elapsed > INSPECTION_DNF_MS ? 'DNF' : elapsed > INSPECTION_MS ? '+2' : null;
        } else {
    ...
    function hardwareStopped(hardwareTimeMs) {
        if (state !== 'running') return;
        clearInterval(tickInterval);
        const entry = {
            time: hardwareTimeMs, date: new Date().toISOString(), scramble: currentScramble,
            penalty: pendingPenalty, source: 'stackmat',
        };
        session.push(entry);
        saveSession(session);
        setState('stopped');
    ...
    return {
        ...
        hardwareReady, hardwareStarted, hardwareStopped, hardwareReset,
